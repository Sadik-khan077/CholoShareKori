...
const Borrow = () => {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <p style={{ color: 'var(--text-muted)', marginBottom: '2rem' }}>Find items available to borrow from your peers.</p>
      ...
      <div className="bento-card" style={{ 
        ...
        <select style={{...controlStyle, flex: '1', minWidth: '180px'}} value={sortBy} onChange={(e) => setSortBy(e.target.value)}>
          ...
          <option value="price-asc" style={optionStyle}>Fee: Low to High</option>
          <option value="price-desc" style={optionStyle}>Fee: High to Low</option>
        </select>
      </div>
          ...
          processedResources.map(resource => (
            <div key={resource.id} style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ marginTop: '0.75rem', display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem' }}>
                ...
                <span>
                  Lent by: <Link to={`/profile/${resource.user_id}`} style={{ color: 'var(--text-main)', textDecoration: 'none', fontWeight: 'bold' }}>{resource.user_name || 'User'}</Link>
                </span>
              </div>
            </div>
          ))
        ) : (
          <p style={{ color: 'var(--text-muted)' }}>No items available to borrow right now.</p>
        )}
      </div>
    </div>
  );
